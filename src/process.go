...
/*
В этой функции мы создаем процессы, которые будут запускаться в дальнейшем (при запуске taskmaster)
processSlice - слайс процессов, которые будут запущены. Слайсы это тот же массив.
Проходимся по всем процессам, которые нужно запустить и создаем. При этом, каждый процесс имеет свой конфиг.
Это конфигурационное имя процесса, количество процессов, которые нужно запустить и команда, которую нужно выполнить.
*/
